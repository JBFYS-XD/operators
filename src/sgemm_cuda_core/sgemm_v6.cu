...
// 对 shared 内存和寄存器俩个缓存进行 双缓冲 隐藏延迟

// CUBLAS: 4.505122 ms
// MyKernel: 5.264477 ms
...
void sgemm_gKernel(float* A, float* B, float* C, int M, int N, int K) {
    ...
    for (int i = 0; i < THREAD_X; i += 4) {
        FETCH_FLOAT4(reg_calc_b[0][i]) = FETCH_FLOAT4(smem_B[0][0][col_tid_calc + i]);
    }
    #pragma unroll
    for (int i = 0; i < THREAD_Y; i += 4) {
        FETCH_FLOAT4(reg_calc_a[0][i]) = FETCH_FLOAT4(smem_A[0][0][row_tid_calc + i]);
    }
    ...
    do {
        ...
        for (int k = 0; k < BLOCK_K - 1; k ++) {
            ...
            #pragma unroll
            for (int i = 0; i < THREAD_X; i += 4) {
                FETCH_FLOAT4(reg_calc_b[pr_f ^ 1][i]) = FETCH_FLOAT4(smem_B[pp_f][k + 1][col_tid_calc + i]);
            }
            #pragma unroll
            for (int i = 0; i < THREAD_Y; i += 4) {
                FETCH_FLOAT4(reg_calc_a[pr_f ^ 1][i]) = FETCH_FLOAT4(smem_A[pp_f][k + 1][row_tid_calc + i]);
            }
        ...
        #pragma unroll
        for (int i = 0; i < THREAD_X; i += 4) {
            FETCH_FLOAT4(reg_calc_b[pr_f ^ 1][i]) = FETCH_FLOAT4(smem_B[pp_f ^ 1][0][col_tid_calc + i]);
        }
        #pragma unroll
        for (int i = 0; i < THREAD_Y; i += 4) {
            FETCH_FLOAT4(reg_calc_a[pr_f ^ 1][i]) = FETCH_FLOAT4(smem_A[pp_f ^ 1][0][row_tid_calc + i]);
        }
        #pragma unroll
        for (int i = 0; i < THREAD_Y; i ++) {
            #pragma unroll
